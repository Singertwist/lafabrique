...
def order_create(request):
	...
	if request.method == 'POST':
		try:
			cart_data_validation.picking_date() # Ajout d'une condition afin de s'assurer qu'une date est renseignée dans le panier. Bloque le paiement si cela n'est pas le cas.
			...
			form = OrderCreateForm(request.POST)
			messages.warning(request, "<p>Veuillez une date pour valider votre panier.</p>")
			return render(request, "commandes/orders-create.html", {'cart':cart, 'final_composed_cart':final_composed_cart, 'form':form, 'stripe_pk_key':stripe_pk_key, 'cart_data_validation':cart_data_validation})
