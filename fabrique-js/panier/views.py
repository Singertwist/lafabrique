...
def cart_remove_one(request, product_id):
	...
	if request.is_ajax():
		cart.remove_one(product=product, quantity=1)
...
def add_to_final_composed_cart(request, categorie_composed_cart):
	...
	if request.method == 'POST':
		if request.is_ajax():
			...
			cart_data = final_composed_cart.add_to_final_composed_cart(categorie_composed_cart=categorie_composed_cart, quantity=1, comment=comment)
			return JsonResponse({'data':cart_data[1]})
		else:
			if form.is_valid():
				cd = form.cleaned_data
				next = cd['next']
